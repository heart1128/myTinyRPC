 ...
 * 替代 std::shared_ptr(this) 返回this,这样如果会引起两次析构，因为有两次智能指针，this是裸指针，使用裸指针会构造一个新的智能指针
 * 使用shared_from_this是返回一个weak_ptr.loak()对象，weak_ptr监控的是本类的智能指针，所以不会创建一个新的智能指针。
 * 
